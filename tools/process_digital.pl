...
opendir(DIR, $gl_sourcedir) || die "Cannot scan source directory '$sourcedir'\n";
my %dirlist = ();
my %neflist = ();
...
sub process_photo
{
    ...
    if ($gl_timezoneoffset) {
        if ($timezone =~ /^([+\-]?\d\d):(\d\d)$/) {
            ...
            print "Got timezone offset but don't recognize time zone '$timezone', skip\n";
            return;
        }
    }        
    ...
    if (($setID eq "") || ($targetfile eq "")) {
        ...
        set_database_info($imageid, $do_portrait, $newer_rotate, $latlong, $timezone, $dst, $is_mov, $is_kids);
        ...
        if ($fname =~ /\.jpe?g$/i) {
            ...
            system("exiftool -b -JpgFromRaw $dir/$fname > $set_directory/tif/$imageid.jpg");
            ...
            # Extract the JPG first
            system("exiftool -b -PreviewImage $dir/$fname > $set_directory/tif/$imageid.jpg");
            # Add all the EXIF information to the JPG file
            system("exiftool -TagsFromFile $dir/$fname -q -q -SerialNumber=0 -overwrite_original $set_directory/tif/$imageid.jpg");
...
sub set_database_info {
    ...
    if (!pdb_image_info($imageid)) {
        if ($imageid =~ /^(\d\d\d\d)(\d\d)(\d\d)\-(\d\d)(\d\d)(\d\d)/) {
            ...
            pdb_set_year($year);
            pdb_set_orientation($do_portrait
                                ? $PCOM_PORTRAIT : $PCOM_LANDSCAPE);
            if ($is_kids) {
