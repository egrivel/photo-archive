...
sub pfs_get_size {
    ...
    if ($setdir ne "") {
        ...
        if ((-f "$setdir/jpeg/$imageID.jpg")
            && !((-f "$setdir/edited/$imageID.jpg")
                 || (-f "$setdir/jpeg2/$imageID.jpg"))) {
            return $PCOM_NORMAL;
        }
        return $PCOM_LARGE;
    }
    ...
    if ($setdir ne "") {
        if (-f "$setdir/edited/$imageID.jpg") {
            ...
            while (-f "$setdir/edited/$imageID-$nr.jpg") {
                if (pfs_get_time("$setdir/edited/$imageID-$nr.jpg")
                    > $ftime) {
...
sub pfs_get_size_location {
    ...
    if ($setdir ne "") {
        ...
        if (-f "$setdir/$size/$imageID.jpg") {
            if (pfs_get_time("$setdir/$size/$imageID.jpg")
                >= $edittime) {
        ...
        if (defined($altsize) && ($altsize ne "")) {
            if (-f "$setdir/$altsize/$imageID.jpg") {
                if (pfs_get_time("$setdir/$altsize/$imageID.jpg")
                    >= $edittime) {
...
sub pfs_get_custom_realsize_location {
    ...
    if ($setdir ne "") {
        ...
        my $fname = "$setdir/custom/$imageID-${width}-${height}-${realwidth}-${realheight}";
...
sub pfs_cmd_copy_exif {
    ...
    if ($origfile =~ /\.nef$/) {
        ...
        $cmd = "exiftool -TagsFromFile $origfile -q -q -SerialNumber=0 -Orientation= -overwrite_original $targetfile";
    }
    return $cmd;
}
...
sub pfs_cmd_resize {
    ...
    my $is_freeform = (($orientation eq $PCOM_FREEFORM)
        ...
        if ($is_freeform) {
            ...
            if ($keep_aspect) {
                ...
                if ($width >= $height) {
                    ...
                    if ($fill_portrait) {
                        pcom_log($PCOM_DEBUG, "freeform, don't keep aspect, portrait, fillportrait=$fill_portrait");
                        $finalsize = "${longval}x${shortval}";
...
#                $page .= " -strip";
    ...
    return "convert -size $finalsize $fname $rotate -resize $scalesize $page -crop $finalsize $realresize $quality $sharpen $remove_profile jpg:- > $outfile; " . pfs_cmd_copy_exif($imageID, $outfile);
}
...
sub pfs_cmd_large_movie {
    ...
    if (($orig ne "") && ($outfile ne "")) {
        $outfile =~ s/\.jpg$/.mp4/;
        return "ffmpeg -i $orig -vcodec libx264 -acodec libvorbis -aq 5 -ac 2 -qmax 25 -vf scale=960:540 $outfile.mp4; qt-faststart $outfile.mp4 $outfile; rm $outfile.mp4; chmod a+w $outfile";
    }
    return "";
}
