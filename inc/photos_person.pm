...
my @person_fields = ("personid", "fullname", "description",);

my @personref_fields = ("imageid", "personid");
...
sub ppers_get_id {
  ...
  if (($person_id eq "")
    && $add_if_not_exist) {
    my $query =
      "INSERT INTO person (personid, fullname, description) VALUES(UUID(), '$value', '');";
    psql_command($query);
...
sub ppers_update {
  ...
  while ($person_list =~ s/^(.*?);//) {
    ...
    if ($name ne "") {
      ...
      if ($person_id ne "") {
        $query =
          "INSERT INTO personref (imageid, personid) VALUES ('$image_id', '$person_id');";
        psql_command($query);
      }
    }
  }
...
sub ppers_get_person_list {
  ...
  if ($do_count) {
    $query =
      "SELECT personid, fullname, description, (SELECT COUNT(imageid) FROM personref WHERE personref.personid = person.personid) as 'count' FROM person";
  }
  if (psql_command($query)) {
...
sub ppers_get_photos {
  ...
  my $query =
    "SELECT personref.imageid FROM personref INNER JOIN images ON images.imageid = personref.imageid  WHERE personref.personid='"
    . psql_encode($persid)
    . "' ORDER BY images.year, images.sortid;";
  my @result = ();
  my $count = 0;
...
sub ppers_sync_person {
  ...
  while (defined($record)) {
    ...
    if (!defined($images_still_exist{$imageid})) {
      ...
      my $query =
          "DELETE FROM personref WHERE personid='"
        . psql_encode($personid)
        . "' AND imageid='"
        . psql_encode($imageid) . "'";
      psql_command($query);
...
sub ppers_sync_set_person {
  ...
  my $personid = "";
  if ($data =~ s/^database:\s+([^\n]*?)\n//) {
    my $database = $1;
    if ($database =~ /^personid='([\w-]+)'/) {
      $personid = $1;
    } else {
...
sub ppers_sync_del_person {
  ...
  $query = "DELETE FROM person WHERE personid='" . psql_encode($personid) . "'";
  psql_command($query);
